...
import { ProductsApplicationService } from '../../../products/application/services/products.application.service';
import { ClientsApplicationService } from 'src/modules/clients/application/services/clients.application.service';
import { IProduct } from '../../../products/application/interfaces/product.interface';
import { IClient } from 'src/modules/clients/application/interfaces/client-response.interface';
import {
...
export class SalesApplicationService {
  constructor(
    ...
    private readonly productService: ProductsApplicationService,
  ...
  async findAll(
    ...
    const { pageNumber = 1, displayPerPage = 10, dateFrom, dateTo } = findDto;
    ...
    this.validateSalesDateRange(dateFrom, dateTo);

    const [sales, totalNumber] = await this.salesRepository.findSales(
      findDto,
      mode,
      client_ext_id,
    );
    ...
    const enrichedSales = await Promise.all(
      sales.map(async (sale) => {
        const [
          ...
          this.salesRepository.findItemsBySaleExtId(sale.external_id),
          this.salesRepository.findPaymentsBySaleExtId(sale.external_id),
          this.salesRepository.findLayawayBySaleExtId(sale.external_id),
          this.validateClient(sale.client_ext_id),
          this.validateProducts(
            (
              await this.salesRepository.findItemsBySaleExtId(sale.external_id)
  ...
  async findOne(external_id: string): Promise<ISaleResponse> {
    const sales = await this.salesRepository.findSaleByExtId(external_id);
    ...
    const salesItems =
      await this.salesRepository.findItemsBySaleExtId(external_id);
    ...
    const paymentHistory =
      await this.salesRepository.findPaymentsBySaleExtId(external_id);
    ...
    const salesLayaway =
      await this.salesRepository.findLayawayBySaleExtId(external_id);
  ...
  async createSale(dto: SalesDto): Promise<ISaleResponse> {
    ...
    const salesItems = dto.products.map((product) => {
      ...
      return this.salesRepository.createItem({
    ...
    const sales = this.salesRepository.createSale({
      ...
      date_purchased: new Date(dto.date_purchased),
    ...
    const paymentLog = this.salesRepository.createPayment({
      ...
      payment_date: new Date(dto.payment.payment_date),
    ...
    if (dto.type === 'L') {
      ...
      saleLayaway = this.salesRepository.createLayaway({
        sale_ext_id: sale_ext_id,
        no_of_months: dto.layaway.no_of_months,
        amount_due: amountDue,
        payment_date: null,
        current_due_date: new Date(dto.layaway.due_date),
        orig_due_date: new Date(dto.layaway.due_date),
        is_extended: false,
        status: amountDue > 0 ? 'Unpaid' : 'Paid',
        created_by: dto.created_by,
      });

      await this.salesRepository.saveLayaway(saleLayaway);
    ...
    await this.salesRepository.saveSale(sales);
    await this.salesRepository.saveItems(salesItems);
    await this.salesRepository.savePayment(paymentLog);
  ...
  async recordPayment(dto: RecordPaymentDto): Promise<ISaleResponse> {
    const layawaySales = await this.salesRepository.findLayawaySaleByExtId(
      dto.sale_ext_id,
    );
    ...
    let paymentLogsArray = await this.salesRepository.findPaymentsBySaleExtId(
      layawaySales.external_id,
    );

    const salesLayawayDetails =
      await this.salesRepository.findLayawayBySaleExtId(
        layawaySales.external_id,
      );
    ...
    if (Number(outstandingBalance) <= 0) {
      ...
      await this.salesRepository.saveSale(layawaySales);
      ...
      if (salesLayawayDetails) {
        ...
        await this.salesRepository.saveLayaway(salesLayawayDetails);
      }
    ...
    if (isDeposit) {
      ...
      paymentLogsArray = await this.salesRepository.findPaymentsBySaleExtId(
        layawaySales.external_id,
      );
  ...
  async cancelSales(dto: CancelSaleDto): Promise<ISaleResponse> {
    const sales = await this.salesRepository.findActiveSaleByExtId(
      dto.sale_ext_id,
    );
    ...
    const salesItems = await this.salesRepository.findItemsBySaleExtId(
      sales.external_id,
    );
  ...
  async extendLayawayDueDate(
    ...
    const layawaySales =
      await this.salesRepository.findLayawaySaleByExtId(external_id);
  ...
  private validateSalesDateRange(dateFrom?: string, dateTo?: string): void {
    if (dateFrom && dateTo) {
  ...
  async getSalesStats(
    ...
    const data: Partial<ISaleTransactionsResponse['data']> = {};
    ...
    // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
    const buildDateCondition = (): SalesDateRange => {
      if (fromDate && toDate) {
    ...
    if (dateFrom || dateTo) {
      ...
      for (const status of statuses) {
        const result = await this.salesRepository.getSalesStatusAggregate(
          status,
          mode,
          dateCondition,
        );
    ...
    for (const status of statuses) {
      const total = await this.salesRepository.getSalesStatusAggregate(
        status,
        mode,
      );
      const today = await this.salesRepository.getSalesStatusAggregate(
        status,
        mode,
        { start: todayStart },
      );
      const yesterday = await this.salesRepository.getSalesStatusAggregate(
        status,
        mode,
        { start: yesterdayStart, end: todayStart },
      );
      const lastWeek = await this.salesRepository.getSalesStatusAggregate(
        status,
        mode,
        { start: lastWeekStart },
      );
      const lastMonth = await this.salesRepository.getSalesStatusAggregate(
        status,
        mode,
        { start: lastMonthStart },
      );
      const lastYear = await this.salesRepository.getSalesStatusAggregate(
        status,
        mode,
        { start: lastYearStart },
      );
  ...
  async getCustomerPurchaseFrequency(
    ...
    const buildMetrics = async (
      ...
      const allSales =
        await this.salesRepository.getCustomerPurchaseFrequencies(from, to);

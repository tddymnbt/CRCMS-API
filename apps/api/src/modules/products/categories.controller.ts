...
export class CategoriesController {
  constructor(
    private readonly service: CategoriesApplicationService,
    private loggerService: ActivityLogsApplicationService,
  ) {}

  @Get()
  @ApiOperation({ summary: 'Find all product category records' })
  @ApiOkResponse({
    description: 'List of product category records',
  ...
  @ApiBusinessError(
    404,
    'No product category exists with the given external id',
  )
  ...
  @Post()
  @ApiOperation({ summary: 'Create product category record' })
  @ApiCreatedResponse({
    description: 'product category successfully created',
    type: ProductMiscResponseDto,
  })
  @ApiValidationError()
  @ApiBusinessError(409, 'A product category with the same name already exists')
  async create(
